    ...
    private render(): void {
        ...
        this.el.innerHTML = `
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; width: 100%;">
                ${presets.map((m, idx) => `
                    <div style="background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); padding: 22px; display: flex; flex-direction: column; gap: 14px; transition: all 0.2s ease;" onmouseenter="this.style.borderColor='var(--border-medium)'; this.style.transform='translateY(-2px)'" onmouseleave="this.style.borderColor='var(--border-subtle)'; this.style.transform='none'">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                            <div>
                                ...
                                <h3 style="font-size: 16px; font-weight: 700; color: var(--text-primary); margin-top: 2px;">${m.name}</h3>
                            </div>
                        ...
                        <p style="font-size: 12px; color: var(--text-secondary); line-height: 1.5; min-height: 36px;">${m.description}</p>

                        <div style="border-top: 1px solid var(--border-subtle); padding-top: 12px; display: flex; flex-direction: column; gap: 6px; font-size: 11px;">
                        ...
                        <button class="btn btn-secondary btn-sm load-mat-btn" data-mat="${m.id}" style="margin-top: 6px; width: 100%;">
                            Simulate Specimen →
                        </button>
                    </div>
                `).join("")}
